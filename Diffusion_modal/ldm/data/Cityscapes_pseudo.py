...
class CityscapesBase(Dataset):
    def __init__(self,
        ...
        self.train_or_val = 'val' if 'val' in txt_file else 'train'
    ...
    def __getitem__(self, i):
        ...
        if self.train_or_val == 'train':
            path = os.path.join(self.data_root, self.image_paths[i].split(' ')[0])
            path = os.path.join(self.data_root, 'leftImg8bit', self.train_or_val, self.image_paths[i].split(' ')[0])
            pil_image = Image.open(path)
            if not pil_image.mode == "RGB":
                pil_image = pil_image.convert("RGB")

            #path2 = os.path.join(self.data_root, self.image_paths[i].split(' ')[1]) # add replace
